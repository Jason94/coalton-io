...
if too many tasks are submitted."
  ...
  (inline)
  (declare work-queue% (UnboundedScheduler :a -> UnboundedMpmcQueue :a))
  (define (work-queue% (UnboundedScheduler% work-queue))
    work-queue)

  (define-instance (Scheduler UnboundedScheduler)
    (inline)
    (define (submit item scheduler)
      (enqueue item (work-queue% scheduler)))
    (inline)
