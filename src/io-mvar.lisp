...
or just release the LOCK."
  ...
  (define (take-mvar% mvar)
    (wrap-io
      (let x = (lisp Integer ()
                 (cl:random 100)))
      (write-line-sync% (build-str "[" x "] Starting take-mvar%"))
      (mask-current-thread!%)
      (lk:acquire (.lock mvar))
      (write-line-sync% (build-str "[" x "] Acquired lock take-mvar%"))
      (let ((lp (fn ()
                  (match (c:read (.data mvar))
                    ((Some val)
                     (write-line-sync% (build-str "[" x "] Took val " (force-string val) " take-mvar%"))
                     (c:write! (.data mvar) None)
                     (lk:release (.lock mvar))
                     (cv:notify (.cvar mvar))
                     (write-line-sync% (build-str "[" x "] Preparing unmask-current thread take-mvar%"))
                     (unmask-current-thread!%)
                     (write-line-sync% (build-str "[" x "] Finished unmask-current thread take-mvar%"))
                     val)
                    ((None)
                     (write-line-sync% (build-str "[" x "] Preparing unmask-and-await take-mvar%"))
                     (unmask-and-await-safely% (.cvar mvar) (.lock mvar))
                     (write-line-sync% (build-str "[" x "] Finished awaiting take-mvar%"))
                     (mask-current-thread!%)
                     (write-line-sync% (build-str "[" x "] Finished masking take-mvar%"))
                     (lp))))))
        (lp))))
