...
(coalton-toplevel
  ...
  (define-instance (Runtime IoRuntime IoThread)
    ...
    (inline)
    (define (park-current-thread-if! prx with-gen should-park?)
      (park-current-thread-if!% prx with-gen should-park?))
    (inline)
    (define (park-current-thread-if-with! prx with-gen should-park? strategy)
      (park-current-thread-if-with!% prx with-gen should-park? strategy))
    (inline)
