...
(coalton-toplevel
  ...
  (define-instance (Runtime IoRuntime IoThread)
    ...
    (define (mask! _ thread)
      (mask!% thread))
    (inline)
    (define (mask-current! _)
      (mask-current-thread!%))
    (inline)
    (define (unmask! _ thread)
      (unmask!% thread))
    (inline)
    (define (unmask-current! _)
    ...
    (inline)
    (define (park-current-thread-if-masked! prx with-gen should-park? &key (timeout NoTimeout))
      (park-current-thread-if-masked!% prx with-gen should-park? :timeout timeout))
    (inline)
    (define (unpark-thread-masked! _ gen thread)
      (unpark-thread-masked!% gen thread))
    (inline)
